...
export function WateryBackground(){
 ...
 useEffect(()=>{const observer=new IntersectionObserver(entries=>{if(entries.some(e=>e.isIntersecting)){setMounted(true);observer.disconnect();}},{rootMargin:'600px'});['featured-projects','race-sequence'].forEach(id=>{const el=document.getElementById(id);if(el)observer.observe(el);});return()=>observer.disconnect();},[]);
 useEffect(()=>{
  if(!mounted)return;let frame=0,visible=false;
  const zones=[{element:document.getElementById('featured-projects'),input:wateryInputs.featured},{element:document.getElementById('race-sequence'),input:wateryInputs.sequence}];
