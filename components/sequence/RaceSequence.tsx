...
// React uses the server snapshot for both SSR and the first hydration render.
...
function SequenceCard({i,index,enter,progress,reduced,select}:{i:number;index:number;enter:MotionValue<number>;progress:MotionValue<number>;reduced:boolean;select:(index:number)=>void}){
 ...
 const xPercent = round(Math.sin(angle) * 112);
 const zPosition = round((Math.cos(angle) - 1) * 380);
 const rotate = round(-distance * 28);
 ...
 return <motion.div className="sequence-entry" style={{zIndex:6-Math.abs(distance),x:reduced?0:x,y:reduced?0:y,scale:reduced?1:scale,opacity:reduced?1:opacity,rotateY:reduced?0:rotateY}}>
  <motion.button className={`sequence-photo${center?' is-selected':''}`} aria-label={`Select ${titles[i]}`} aria-current={center?"true":undefined} tabIndex={Math.abs(distance)<3?0:-1} style={{zIndex:6-Math.abs(distance),pointerEvents:Math.abs(distance)<3?'auto':'none'}} initial={false} animate={{x:`${xPercent}%`,z:zPosition,scale:center?1:Math.abs(distance)===1?.8:.62,rotateY:rotate,opacity: distance === 0 ? 1 : Math.abs(distance) === 1 ? 1 : Math.abs(distance) === 2 ? 0.82 : 0}} transition={{duration:reduced?0:.72,ease:[.16,1,.3,1]}} onClick={()=>select(i)}>
...
export function RaceSequence(){
 ...
 const reduced=hydrated&&prefersReducedMotion;const section=useRef<HTMLElement>(null),header=useRef<HTMLElement>(null),stage=useRef<HTMLDivElement>(null);
 ...
 return <section ref={section} id="race-sequence" className="race-sequence race-chapter" data-index={index} onPointerMove={e=>{wateryInputs.sequence.x=e.clientX/innerWidth;wateryInputs.sequence.y=1-e.clientY/innerHeight;}}>
  ...
  <div className="sequence-caption" aria-live="polite"><motion.span key={index} className="race-meta" initial={{y:reduced?0:6,opacity:0}} animate={{y:0,opacity:1}} transition={{duration:.3}}>0{index+1} / 06</motion.span><div className="sequence-title-slot"><AnimatePresence mode="wait" initial={false}><motion.h3 key={index} initial={{y:reduced?0:24,opacity:0}} animate={{y:0,opacity:1}} exit={{y:reduced?0:-18,opacity:0}} transition={{duration:reduced?0:.25,ease:[.16,1,.3,1]}}>{titles[index]}</motion.h3></AnimatePresence></div><span className="race-meta">MACHINE / MOTION / LIGHT</span></div>
